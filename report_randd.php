...
function get_field_match_private($data)
{
  ...
  if (!isset($mrbs_user) || ($mrbs_user->level == 0)) {
    ...
    $options = array(
      BOOLEAN_MATCH_BOTH  => get_vocab('both'),
  ...
  foreach ($field_order_list as $field) {
    ...
    switch ($field) {
      ...
      case 'approval_enabled':
        ...
        break;
      case 'last_updated':
        $values[] = type_wrap(get_vocab("request_type"), 'title-numeric');
        break;
      case 'status':
        $values[] = get_vocab("status");
        break;
      default:
        // the custom fields
      ...
      default:
        ...
        if (array_key_exists($field, $custom_fields)) {
          ...
          elseif (isset($value)) {
            ...
            // an empty string)
            if (
              isset($select_options["entry.$field"]) &&
...
function get_match_condition($full_column_name, $match, &$sql_params)
{
  ...
  if (
    isset($select_options["$table.$column"]) &&
...
$statusmatch = get_form_var('statusmatch', 'int');
...
// Build the report search field order
$report_search_fields = array(
  'report_start', 'report_end',
  'areamatch', 'roommatch',
  'typematch', 'namematch', 'descrmatch', 'creatormatch',
  'match_private', 'match_confirmed', 'match_approved', 'statusmatch'
);
...
$field_order_list = array(
  'name', 'area_name', 'room_name', 'start_time', 'end_time',
  'description', 'type', 'allow_registration', 'create_by', 'confirmation_enabled',
  'approval_enabled', 'status'
);
foreach ($custom_fields as $key => $value) {
...
if ($output_form) {
  ...
  // Search variables
  $search_var_keys = array(
    'from_day', 'from_month', 'from_year',
    'to_day', 'to_month', 'to_year',
    'areamatch', 'roommatch',
    'typematch', 'namematch', 'descrmatch', 'statusmatch', 'creatormatch',
    'match_private', 'match_confirmed', 'match_approved',
    'custom_fields', 'statusmatch'
  );
  $search_vars = array();
