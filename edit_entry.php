...
$entry_fields = array(
  'create_by', 'name', 'description', 'start_time', 'end_time', 'room_id',
...
function get_field_description($value, $disabled = false)
{
  ...
  $params = array(
    'label'    => get_vocab('fulldescription'),
    'name'     => 'description',
...
function get_field_start_time($value, $disabled = false)
{
  ...
  foreach ($areas as $a) {
    $field->addControlElement(get_slot_selector(
      $a,
      'start_seconds' . $a['id'],
      'start_seconds',
      $current_s,
...
function get_fieldset_rep_weekly_details($disabled = false)
{
  ...
  $fieldset->setAttributes(array(
    'class' => 'rep_type_details js_none',
    'id'    => 'rep_weekly'
  ));
