...
function output_row($row, $returl)
{
  ...
  if ($row['status'] == 0) {
    ...
    $status = "<b style='color: green; font-size:10pt;'>Approved</b>";
  } elseif ($row['status'] == 2) {
    $status = "<b  style='color: red; font-size:10pt;'>Reject</b>";
  } elseif ($row['status'] == 4) {
    $status = "<b  style='color: red; font-size:10pt;'>Tentative</b>";
  }
  $values[] = $status;
...
// print_r($mrbs_user->username); exit;
...
foreach ($fields as $field) {
  if (!in_array($field['name'], $standard_fields['entry'])) {
    ...
    if (
      isset($select_options["entry." . $field['name']]) &&
...
if (!$ajax_capable || $is_ajax) {
  ...
  $sql = "SELECT E.id AS entry_id, E.create_by, E.name, E.description, E.start_time,E.status,
