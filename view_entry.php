...
function generate_registrant_table($row, $previous_page = null)
{
  ...
  foreach ($row['registrants'] as $registrant) {
    ...
    if (
      isset($registrant_creator) && isset($registrant_user) &&
...
function generate_cancel_registration_button(array $row, array $registrant, $label_text, $previous_page = null, $as_field = false)
{
  ...
  $form->setAttributes(array(
    'action' => multisite('registration_handler.php'),
    'method' => 'post'
  ));
...
function generate_event_registration($row, $previous_page = null)
{
  ...
  $mrbs_user = session()->getCurrentUser();
  if (!$can_register_others && in_arrayi(
    $mrbs_user->username,
    ...
    if (
      empty($row['registrant_limit_enabled']) ||
...
function generateApproveButtons($id, $series)
{
  ...
  // Approve
  $params = array(
    'action' => multisite("approve_entry_handler.php?$query_string"),
  ...
  // Reject
  $params = array(
    'action' => multisite("$this_page?$query_string"),
...
//print_r($username); exit;
    ...
    if ($row['status'] == 0 && $row['create_by'] == $username) { 
      ...
      if ((!empty($repeat_id) || $series) && $repeats_allowed) {
        ...
        if (
          empty($button_attributes['disabled']) &&
          isset($repeat_id) &&
